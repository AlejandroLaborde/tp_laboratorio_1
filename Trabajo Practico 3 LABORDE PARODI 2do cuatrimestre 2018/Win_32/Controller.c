...
int controller_editEmployee(LinkedList* pArrayListEmployee)
{
   ...
   if(controller_ListEmployee(pArrayListEmployee))
   {
    ...
    EmpleadoAModificar=ll_get(pArrayListEmployee,Index-1);
...
int controller_ListEmployee(LinkedList* pArrayListEmployee)
{
    ...
    if(emplead != NULL){
        for(i=0;i<ll_len(pArrayListEmployee);i++){
            emplead=ll_get(pArrayListEmployee,i);
            printf("%6d%15s%10d%15d\n",emplead->id,emplead->nombre,emplead->horasTrabajadas,emplead->sueldo);
...
int controller_saveAsBinary(char* path , LinkedList* pArrayListEmployee)
{
    ...
    if(file != NULL && (ll_len(pArrayListEmployee)!=0) )
    {
        ...
        {
            emplead=ll_get(pArrayListEmployee,i);
            aux=fwrite(emplead,sizeof(Employee),1,file);
        }
        printf("El archivo se salvo correctamente\n");
    }
    else
